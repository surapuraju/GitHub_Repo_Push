...
# Extract filename and directory for GitHub
file_name = os.path.basename(FILE_PATH)
repo_directory = os.path.dirname(get_config_value("DEFAULT", "FilePath", "Data/"))

# Debugging: Print resolved paths
print(f"📂 Resolved file path: {FILE_PATH}")
print(f"📁 Uploading to repo directory: {repo_directory}")
...
# GitHub API URL for uploading file inside `Data/` folder
GITHUB_API_URL = f"https://api.github.com/repos/{USERNAME}/{REPO}/contents/{repo_directory}/{file_name}"

# Debugging: Print GitHub API request details
print(f"🛠️ API URL: {GITHUB_API_URL}")
print(f"🔑 Using token: {API_TOKEN[:4]}... (hidden for security)")
print(f"📂 Target branch: {BRANCH}")
print(f"📜 File path in repo: {repo_directory}/{file_name}")
...
if response.status_code == 200:
    ...
    sha = None  # New file
    print(f"🆕 New file: {repo_directory}/{file_name}")

# Commit message
commit_message = f"Adding or updating {repo_directory}/{file_name} via API"
